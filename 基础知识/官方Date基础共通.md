...
# **一. 核心开始**
- **所有的各种Type变量**都需要先在这里定义的**实际类型**也就是KIND
## **Kinds**
```xml
...
- 我更喜欢称这里的Tag标签为大类划分，将一部分Type变量赋予统一的标签，成为一个大类Class
```xml
<!-- PS: Tags表是一个定义标签的表，拥有给其他各种Type变量进行标签，用于对拥有相同标签的Type变量对象进行统一的操作，
	 例如对多个单位增加一个陆军tag标签，赋予相同的单位能力或是AI行为逻辑 -->
```
| 表可填参数 | 值类型 | 默认值 | 注释
| -- | -- | :--: | --
...
- Type变量们可以赋予多个Tag，例如开括者就即是陆地单位类的又可以是平民单位类的
```xml
<!-- PS: TypeTags表是对Type变量赋予标签Tag，注意这不是必须的，Type变量可不标记标签
          拥有相同Tag的Type变量将属于同一个大标签下的大类别，方便我们对有这个标签的Type变量进行
          统一的操作修改，而不是一个一个Type变量进行赋予相同的东西 -->
```
| 表可填参数 | 值类型 | 默认值 | 注释
| -- | -- | :--: | --
| Type="各种Type变量" | TEXT:star: | NULL |
