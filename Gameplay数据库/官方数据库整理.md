...
| Ethnicity="文明种族类型" | TEXT | NULL | 这个决定文明是什么大类种族

- **RandomCityNameDepth**：这个机制是这样的首先你在CityNames表定义30个城市名字并绑到了这个文明，那么这个文明就有了一个城市名称库
...
### CivilizationLeaders
- **文明**和**领袖**绑一起的表，顺带还包括定义这个领袖首都的功能
```
...
<CivilizationLeaders
	...
	CapitalName="首都城市文本"		值类型="TEXT"		默认值="NULL"	注释="这里填入“从定义的城市库CityNames”里选出首都名称的文本变量"
/>
```

| 表可填参数 | 值类型 | 默认值 | 注释
| -- | -- | :--: | --
| CivilizationType="文明Type变量" | TEXT | NULL |
| LeaderType="领袖Type变量" | TEXT | NULL |
| CapitalName="首都城市文本" | TEXT | NULL | 这里填入“从定义的城市库”里选出首都名称的文本变量
...
<CityNames
	ID="城市名称ID?"				值类型="INTEGER"	默认值="NULL"	注释="这个不用我们定义会自动生成的，定义会不会有问题，未知?"
	CivilizationType="文明Type变量"	值类型="TEXT"		默认值="NULL"	注释="官方都是绑的文明"
	LeaderType="领袖Type变量"		值类型="TEXT"		默认值="NULL"	注释="这个都没用的官方全为默认值NULL，或许可以绑领袖建立城市名称库，有待测试"
	CityName="文明Type变量"			X值类型="TEXT"		默认值="NULL"
	ContinentType="大陆类型"		值类型="TEXT"		默认值="NULL"	注释="官方貌似全是默认值NULL，有待进一步研究"
	SortIndex="排序索引"			值类型="INTEGER"	默认值="0"		注释="这个应该是没用的官方全为默认值0"
/>
```
...
| CityName="文明Type变量" | TEXT:star: | NULL |
